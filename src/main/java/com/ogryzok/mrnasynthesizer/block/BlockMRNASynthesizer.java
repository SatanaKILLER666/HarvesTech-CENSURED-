package com.ogryzok.mrnasynthesizer.block;
...
public class BlockMRNASynthesizer extends Block {
    ...
    public BlockMRNASynthesizer() {
        super(Material.IRON);
        setRegistryName(MRNASynthesizerRegistry.MODID, "mrna_synthesizer");
        setTranslationKey(MRNASynthesizerRegistry.MODID + ".mrna_synthesizer");
        setHardness(5.0F);
        setResistance(12.0F);
    ...
    @Override public TileEntity createTileEntity(World world, IBlockState state) { return new TileMRNASynthesizer(); }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote) {
            ...
            if (te instanceof TileMRNASynthesizer) {
                ((TileMRNASynthesizer) te).markForSync();
            }
        }
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            player.openGui(MRNASynthesizerRegistry.getModInstance(), MRNASynthesizerRegistry.GUI_ID, world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
