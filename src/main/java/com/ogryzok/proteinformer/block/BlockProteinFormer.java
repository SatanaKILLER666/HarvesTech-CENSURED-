package com.ogryzok.proteinformer.block;
...
public class BlockProteinFormer extends Block {
    private static final AxisAlignedBB ZERO_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D);
    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
    private static final AxisAlignedBB AABB = FULL_BLOCK_AABB;

    public BlockProteinFormer() {
        super(Material.IRON);
        setRegistryName(ProteinFormerRegistry.MODID, "protein_former");
        setTranslationKey(ProteinFormerRegistry.MODID + ".protein_former");
        setHardness(5.0F);
        setResistance(10.0F);
    ...
    @Override public TileEntity createTileEntity(World world, IBlockState state) { return new TileProteinFormer(); }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote) {
            ...
            if (te instanceof TileProteinFormer) {
                ((TileProteinFormer) te).markForSync();
            }
        }
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            player.openGui(ProteinFormerRegistry.getModInstance(), ProteinFormerRegistry.GUI_ID, world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
