...
public class ProteinFormerRenderer extends GeoBlockRenderer<TileProteinFormer> {
    private enum RenderPass { SOLID, GLASS }
    private RenderPass renderPass = RenderPass.SOLID;

    public ProteinFormerRenderer() {
        super(new ProteinFormerGeoModel());
    }

    @Override
    public void render(TileProteinFormer tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        try {
