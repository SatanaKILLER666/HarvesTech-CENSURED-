...
public class BlenderRenderer extends GeoBlockRenderer<TileBlender> {
    private enum RenderPass { SOLID, GLASS }
    private RenderPass renderPass = RenderPass.SOLID;

    public BlenderRenderer() {
        super(new BlenderGeoModel());
    }

    @Override
    public void render(TileBlender tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        try {
