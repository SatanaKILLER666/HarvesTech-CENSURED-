package com.ogryzok.semenenrichment.block;
...
public class BlockSemenEnrichmentChamber extends Block {
    ...
    public BlockSemenEnrichmentChamber() {
        super(Material.IRON);
        setRegistryName(SemenEnrichmentRegistry.MODID, "semen_enrichment_chamber");
        setTranslationKey(SemenEnrichmentRegistry.MODID + ".semen_enrichment_chamber");
        setHardness(5.0F);
        setResistance(10.0F);
    ...
    @Override public TileEntity createTileEntity(World world, IBlockState state) { return new TileSemenEnrichmentChamber(); }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote) {
            ...
            if (te instanceof TileSemenEnrichmentChamber) {
                ((TileSemenEnrichmentChamber) te).markForSync();
            }
        }
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!world.isRemote) {
            player.openGui(SemenEnrichmentRegistry.getModInstance(), SemenEnrichmentRegistry.GUI_ID, world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
