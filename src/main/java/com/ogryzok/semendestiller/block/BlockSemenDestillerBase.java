...
public class BlockSemenDestillerBase extends Block {
    private static final AxisAlignedBB ZERO_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D);
    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
    private static final AxisAlignedBB AABB = new AxisAlignedBB(0,0,0,1,1,1);

    public BlockSemenDestillerBase() {
        super(Material.IRON);
        setRegistryName(SemenDestillerRegistry.MODID, "semen_destiller_base");
        setTranslationKey(SemenDestillerRegistry.MODID + ".semen_destiller_base");
        setHardness(5.0F);
        setResistance(10.0F);
    ...
    @Override public TileEntity createTileEntity(World world, IBlockState state) { return new TileSemenDestillerBase(); }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote) {
            ...
            if (te instanceof TileSemenDestillerBase) ((TileSemenDestillerBase) te).syncStructureState();
            TileEntity above = world.getTileEntity(pos.up());
            if (above instanceof TileSemenDestillerMotor) {
                world.setBlockState(pos.up(), world.getBlockState(pos.up()).withProperty(BlockSemenDestillerMotor.FACING, state.getValue(FACING)), 3);
                ((TileSemenDestillerMotor) above).syncStructureState();
            }
        }
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        TileEntity te = world.getTileEntity(pos);
        if (!(te instanceof TileSemenDestillerBase)) return false;
        TileSemenDestillerBase base = (TileSemenDestillerBase) te;
        if (!world.isRemote) {
            if (!base.isAssembled()) {
                player.sendStatusMessage(new TextComponentString("The destiller only works when assembled: base below and motor above"), true);
                return true;
            }
            player.openGui(SemenDestillerRegistry.getModInstance(), SemenDestillerRegistry.GUI_ID, world, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }

    @Override public void neighborChanged(IBlockState state, World world, BlockPos pos, Block blockIn, BlockPos fromPos) { if (!world.isRemote) { TileEntity te = world.getTileEntity(pos); if (te instanceof TileSemenDestillerBase) ((TileSemenDestillerBase) te).syncStructureState(); }}
